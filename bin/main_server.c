...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/07/19 15:42:49 by ntrancha          #+#    #+#             */
/*   Updated: 2015/07/20 23:35:37 by ntrancha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void app(char *port)
{
		...
		while(1)
		{
				...
				else if(FD_ISSET(sock, &rdfs))
				{
						...
						actual++;
						code = ft_strdup("Code pour l'utilisateur ");
						code2 = ft_strjoin(code, buffer);
						write_socket(c.sock, code2);
						ft_strdel(&code);
						ft_strdel(&code2);
						if(read_socket(csock, buffer) == -1)
						{
						...
						if (ft_strcmp(buffer, "42") == 0)
						{
							printf("OK:%s\n", buffer);
							write_socket(c.sock, "CODE CORRECT");
						}
						else
						{
							printf("ERROR:%s\n", buffer);
							write_socket(c.sock, "CODE ERROR");
...
static void send_message_to_all_clients(Client *clients, Client sender, int actual, const char *buffer, char from_server)
{
		...
		for(i = 0; i < actual; i++)
		{
				...
				if(sender.sock != clients[i].sock)
				{
						...
						write_socket(clients[i].sock, message);
				}
		}
}
